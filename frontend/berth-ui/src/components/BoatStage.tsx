...
export default function BoatStageTop({ calls, playMs }: Props) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    return calls
      ...
      .sort(
        (a, b) => toDate(a.arrival).getTime() - toDate(b.arrival).getTime()
      );
  }, [calls]);
    ...
    >
      ...
      {berths.map((b, i) => (
        ...
        >
          ...
          />
          <Text textAlign="center" fontSize="sm" pt="10px" fontWeight="bold">
            {b}
          </Text>
        </Box>
      ))}
      ...
      <Box
        ...
        left="64px"
        bottom="8px"
        h={`${Math.min(queueIds.length, 5) * 48}px`}
      ...
      >
        ...
        <Text fontSize="sm" fontWeight="bold" w="30px" textAlign="center">
          {speed.toFixed(1)}×
        </Text>
        <Button
          size="xs"
